...
服务器可以直接向客户端发送数据，客户端也可以轮询服务器获取数据。但为了让客户端知道应该向服务器请求什么数据，它需要了解GATT服务器提供的服务和特性。因此，客户端会在连接开始时执行服务发现，以便在执行任何访问操作之前了解服务器的服务和特性。
* Service discovery:GATT客户端发现GATT服务器托管的属性表中的服务和特征的过程

### Data access
需要注意的是，这里的通信基于客户端-服务器架构，服务器持有数据，可以直接将数据发送给客户端，或者客户端可以从服务器轮询数据。因此，GATT操作分为客户端发起的操作和服务器发起的操作。

#### Client-initiated operations
客户端发起的操作是 GATT 操作，其中客户端向 GATT 服务器请求数据。客户端可以请求读取或写入属性，如果是写入操作，则可以选择是否接收来自服务器的确认。

#### Read
如果客户端想要读取存储在 GATT 服务器上某个属性中的特定值，则客户端会向服务器发送读取请求。服务器会响应此请求，并返回该属性值。

#### Write
如果客户端希望向某个属性写入特定值，它会发送写入请求并提供与目标属性格式匹配的数据。如果服务器接受写入操作，它会发送确认响应。

#### Write without response
启用此操作后，客户端无需等待服务器确认即可向属性写入数据。这是一种无需确认的写入操作，可用于需要快速数据交换的场合。

#### Server-initiated operations
GATT操作的另一类是服务器发起的操作，即服务器直接向客户端发送信息，而无需事先收到请求。在这种情况下，服务器可以发出通知或指示。

#### Notify
服务器使用通知操作自动将特定属性的值推送给客户端，而无需客户端请求。例如，这可以用于通知客户端某个传感器读数最近发生了变化。通知无需客户确认。

#### Indicate
与通知操作类似，指示操作也会将属性值直接推送给客户端。但是，在这种情况下，需要客户端的确认。由于需要确认，每个连接间隔只能发送一次指示，这意味着指示比通知慢。虽然这些操作由服务器发起，但客户端需要先订阅相应的特征并启用通知或指示，才能执行这些操作。

## Services and characteristics
ATT 层定义了属性以及客户端和服务器之间的数据公开方式。因此，GATT 的主要功能之一是将存储在 GATT 服务器中的属性分层构建为标准化的实体（服务和特征），从而实现不同蓝牙低功耗设备之间的无缝互操作性。

### Attributes
ATT 层定义了数据在服务器数据库中的存储和访问方式。数据以称为属性的数据结构的形式存储。属性是 ATT 层和 GATT 层共同构建的核心数据单元。属性包含用户数据以及描述属性本身、其类型、安全权限等的元数据。ATT 服务器与客户端之间（或 GATT 服务器与客户端之间）的数据交换以属性的形式进行。如果仅讨论属性，则称其存储在 ATT 服务器中。

一个属性由 4 个数据块组成。
   ![](image/BLE/data_store_in_attribute.png "data_store_in_attribute")

* Handle:属性表中特定属性的 16 位唯一索引由堆栈分配。属性通过其句柄进行寻址。
* Type(UUID):通用唯一标识符 (UUID) 用于指示属性类型。例如，如果此属性声明了一个特征，则其“类型”字段将反映出这一点，因为它包含一个专门用于指示声明特征的 UUID。
* Permissions:处理该属性所需的安全级别（加密和/或授权），以及该属性是否可读和/或可写。
* Value:
   * 属性中存储的实际用户数据（例如：传感器读数）。此字段接受任何数据类型。它可以保存心率监测值（每分钟心跳数）、温度读数，甚至是字符串。
   * 它还可以包含关于另一个属性的信息（metadata）

### Universally unique ID (UUID)
UUID 是蓝牙低功耗 (Bluetooth LE) 领域中经常看到的缩写。它是一个用于标识属性的唯一编号，并告诉我们这些属性的意义。UUID 有两种类型。

第一种类型是 SIG 定义的 16 位 UUID。例如，SIG 定义的心率服务的 UUID 为 0x180D，而其包含的特征之一——心率测量特征——的 UUID 为 0x2A37。16 位 UUID 节能且节省内存，但由于其提供的唯一 ID 数量相对有限，因此需要更多 UUID 来覆盖所有供应商、用户和用例。

第二种类型是 128 位 UUID，有时也称为厂商特定 UUID。创建自定义服务和特征时，需要使用这种类型的 UUID。它类似于：4A98-xxxx-1CC4-E7C1-C757-F1267DD021E8，被称为“基本 UUID”。这四个“x”代表一个字段，您可以在其中插入自定义服务和特征的 16 位 ID，并像使用预定义的 UUID 一样使用它们。这样，您只需将基本 UUID 存储在内存中一次，即可无需再管它，并像往常一样使用 16 位 ID。

### Services
如下图所示，属性是服务的主要组成部分。服务定义（通常简称为服务）由多个属性组成，这些属性按照 GATT 规定的格式排列，从而促进蓝牙低功耗设备之间的标准化数据交换。
   ![](image/BLE/service_definition.png "service_definition")

#### Service declaration attribute
服务定义始终以服务声明属性开头。此属性包含有关服务的元数据，并且还指示服务在 GATT 服务器上存储的服务序列中的起始位置。

   ![](image/BLE/Service_declaration_attribute.png "Service_declaration_attribute")
